...
--
-- Host: localhost:3306
-- Generation Time: Jul 18, 2023 at 04:14 AM
...
CREATE TRIGGER `update_leave_balance` AFTER UPDATE ON `tb_leave_request` FOR EACH ROW BEGIN
    ...
    IF NEW.current_status = 2 THEN
        ...
        SET last_year = IF(leave_duration <= last_year, last_year - leave_duration, 0),
            current_year = IF(leave_duration > last_year, (last_year + current_year) - leave_duration, current_year)
